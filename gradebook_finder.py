...
def call_openai_json(system_prompt: str, user_payload: dict) -> dict:
    try:
        from openai import OpenAI
    except ImportError as exc:
        raise RuntimeError("OpenAI fallback requires the 'openai' package to be installed.") from exc

    client = OpenAI()
    response = client.responses.create(
        model=os.getenv("STUDY_ANALYSIS_LLM_MODEL", "gpt-5-mini"),
        instructions=system_prompt,
        input=json.dumps(user_payload, ensure_ascii=False),
        text={
            "format": {
                "type": "json_schema",
                "name": "gradebook_link_match",
                "strict": True,
                "schema": {
                    "type": "object",
                    "properties": {
                        "selected_link": {"type": ["string", "null"]},
                        "match_type": {
                            "type": "string",
                            "enum": ["group_specific", "shared", "none"],
                        },
                        "reason": {"type": "string"},
                    },
                    "required": ["selected_link", "match_type", "reason"],
...
def find_exact_group_sheet(raw_text: str, group_code: str) -> tuple[str | None, str]:
    exact_match = extract_exact_group_sheets(raw_text).get(group_code)
    if exact_match:
        return exact_match, "wiki_regex"

    bucket_match = extract_bucket_group_sheets(raw_text).get(group_code.split("-")[0])
    if bucket_match:
        return bucket_match, "wiki_bucket_regex"
...
def find_gradebook_on_subject_page(
    *,
    ...
    exact_link, exact_source = find_exact_group_sheet(raw_text, group_code)
    if exact_link:
...
def collect_program_gradebooks(
    ...
    for subject_page in subject_pages:
        ...
        bucket_links = extract_bucket_group_sheets(raw_text)
        ...
        subject_payloads.append(
            {
                ...
                "bucket_group_links": bucket_links,
...
def find_group_gradebooks(
    ...
    seen_subjects = {match.subject_page_title for match in matches}

    for subject_payload in collected["subject_payloads"]:
        if subject_payload["subject_page_title"] in seen_subjects:
            continue
        bucket_link = subject_payload["bucket_group_links"].get(group_code.split("-")[0])
        if not bucket_link:
            continue
        matches.append(
            GradebookMatch(
                subject_name=subject_payload["subject_name"],
                subject_page_title=subject_payload["subject_page_title"],
                subject_page_url=subject_payload["subject_page_url"],
                group=group_code,
                google_sheet_url=bucket_link,
                match_type="group_specific",
                source="wiki_bucket_regex",
                reason=f"На странице найдена общая ведомость для потока {group_code.split('-')[0]}.",
            )
        )
        seen_subjects.add(subject_payload["subject_page_title"])
    ...
    if use_gpt and has_openai_api_key():
        ...
        for subject_payload in collected["subject_payloads"]:
            ...
            exact_link, _ = find_exact_group_sheet(raw_text, group_code)
            if exact_link:
                continue
...
def main() -> None:
    ...
    parser.add_argument("--subject", help="Опционально: искать ведомость только для конкретного предмета")
    ...
    if args.all_groups:
        ...
        if not args.group:
            raise SystemExit("Для поиска по предмету передай --group.")
